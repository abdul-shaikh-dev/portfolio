...
| Financial API memory optimisation | The technical action is clear; the operational effect is not. | Reduced memory usage in financial APIs by optimising LINQ query shape and related data-processing paths. Add a measured reduction or stability result if one is available. |
