...
for role in current['roles']:
    ...
    highlights = '<ul class="role-contributions">' + ''.join('<li>'+b+'</li>' for b in bullets[:2]) + '</ul>'
    more = '<details class="career-details"><summary>More contributions as '+e(role['designation'])+' <span aria-hidden="true">+</span></summary><ul>' + ''.join('<li>'+b+'</li>' for b in bullets[2:]) + '</ul></details>' if len(bullets)>2 else ''
    role_progression += f'<article class="progression-role"><p class="role-date">{e(role["period"])}</p><h4>{e(role["title"])}</h4><p class="progression-title">{e(role["designation"])}</p><div class="progression-contributions">{highlights}{more}</div></article>'
