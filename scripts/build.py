...
service_estate = {
    ...
    'detail': by_id['service-modernisation']['detail'] + ' I also turned the recurring application-chart, infrastructure-chart, and Flux setup into a guided workflow that creates, validates, and publishes the required repositories using the firm’s conventions.',
...
for group_id, label, items in delivery_groups:
    ...
    for p in items:
        ...
        engineering_delivery += f'<article class="delivery-project" id="project-{key}">{aliases}<div class="delivery-title"><h4>{e(p["shortTitle"])}</h4>{badge}</div>{proof}<p>{e(p["summary"])}</p>{figures.get(key, "")}{evidence}{tags(p["tags"])}{technical_notes(p)}</article>'
