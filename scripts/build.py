...
portfolio_map = ''.join(
    f'<a href="{e(item["href"])}"><i aria-hidden="true">{e(item["index"])}</i>'
    f'<span>{e(item["title"])}</span><small>{e(item["subtitle"])}</small></a>'
...
<nav class="work-directory wrap" aria-label="Portfolio map">{portfolio_map}</nav>
